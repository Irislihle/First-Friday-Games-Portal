...
setSaving(true)
try{
    await addPoints(modalTeam,amount)//API
    setTeams((prev) =>
    prev.map((t) => (t.name === modalTeam ? {...t,points:t.points + amount} : t))
    )
    setModalTeam(null)
    setPointsInput('1')
}finally{
...
return(
    <div className='mt-5 bg-[#091f2f] rounded-md shadow-card p-5 text-white'>
      <div className='flex items-baseline justify-between mb-4'>
       ...
       <span className='inline-flex items-center gap-1.5 bg-cyan/15 text-cyan font-mono text-[11px]
       font-bold uppercase tracking-wider px-2.5 py-1 rounded-full'>
    <span className='w-1.5 h-1.5 rounded-full bg-cyan 
    shadow-[0_0_0_3px_rgba(0,184,221,0.25)]' />
            Live
       </span>
     </div> 
    ...
    <div className='flex flex-col gap-2'>
     {
       ...
       <div
          ...
          className='flex items-center justify-between bg-navy-soft rounded-lg pl-3 pr-2.5 py-2.5'>
            <span className='text-[13px] font-semibold text-[#E4E9ED]'>{t.name}</span>
            <span className='flex items-center gap-2.5' >
            <span className='font-mono font-bold text-cyan text-[14px]'>{t.points}</span>
            <button 
               onClick={() => setModalTeam(t.name)}
               disabled={saving === t.name}
               className='bg-red hover:bg-red-deep disabled:opacity-60 text-white
               text-[11px] font-bold rounded-md px-2.5 py-1.5 transition-colors '>
                  + Points
             </button>
          </span>
          </div>
  ...
  <Modal isOpen={!!modalTeam} onClose={() => setModalTeam(null)} title={`Add points to ${modalTeam}`} >
     <form onSubmit={handleSubmit}>
      <input autoFocus type='1' value={pointsInput} min='1' 
